...
#案例2：查询有奖金的每个部门的部门名和领导编号，以及该部门的最低工资
...
#案例1：查询员工名和上级的名称
SELECT e.`employee_id`,e.`last_name`,m.`employee_id` M_e,m.`last_name` M_n
...
#案例2：查询每个工资级别的个数>20的个数，并且按工资级别降序排序
...
WHERE bo.`id` IS NULL; #最好选从表中的主键列（不可能为null）
